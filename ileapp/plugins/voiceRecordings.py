    ...
    def process(self):
        ...
        for plist_file, m4a_file in zip(plist_files, m4a_files):
            with open(plist_file, "rb") as file:
                ...
                ct = unix_epoch_to_readable_date(
                    pl['RCSavedRecordingCreationTime']
                )
                ...
                data_list.append((ct, pl['RCSavedRecordingTitle'],
                                  pl['RCComposedAVURL'].split('//')[1], audio))
...
def unix_epoch_to_readable_date(unix_epoch_time):
    ...
    readable_time = (
        datetime.utcfromtimestamp(unix_time).strftime('%Y-%m-%d %H:%M:%S')
    )
