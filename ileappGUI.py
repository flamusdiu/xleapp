...
from tools.search_files import *
...
def CheckList(mtxt, lkey, mdstring, disable=False):
    ...
    if (mdstring == 'photosMetadata' or mdstring == 'journalStrings'
            or mdstring == 'walStrings'):
        dstate = False
    else:
        dstate = True
    return [sg.CBox(mtxt, default=dstate, key=lkey, metadata=mdstring,
            disabled=disable)]
...
layout = [[sg.Text('iOS Logs, Events, And Plists Parser',
          ...
          [sg.Frame(layout=[
                    ...
                    title=('Select the file type or directory of the target'
                           'iOS full file system extraction for parsing:'))
           ],
          [sg.Frame(layout=[
                    [sg.Input(size=(112, 1)), sg.FolderBrowse(font=normal_font,
                     button_text='Browse Folder')]
                    ],
            ...
            [sg.ProgressBar(max_value=GuiWindow.progress_bar_total,
...
while True:
    ...
    if event == "DESELECT ALL":
         # none modules
        for x in range(1000, indx):
            window[x].Update(False if window[x].metadata != 'lastBuild'
                             else True)  # lastBuild.py is REQUIRED
    if event == 'Process':
        ...
        if is_valid:
            ...
            # hardcode lastBuild as first item
            search_list = {'lastBuild': tosearch['lastBuild']}
            s_items = 0
            for x in range(1000, indx):
                if window.FindElement(x).Get():
                ...
                window[x].Update(disabled is True)
            ...
            crunch_successful = (
                ileapp.crunch_artifacts(search_list,
                                        extracttype,
                                        input_path,
                                        out_params,
                                        len(ileapp.tosearch)/s_items))
